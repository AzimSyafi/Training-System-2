...
def upgrade():
    with app.app_context():
        ...
        db.session.execute('ALTER TABLE "user" ADD COLUMN IF NOT EXISTS role VARCHAR(50) NOT NULL DEFAULT \'agency\';')
        # Backfill any NULLs defensively (older PG might ignore NOT NULL with defaults in some cases)
        db.session.execute("UPDATE \"user\" SET role = 'agency' WHERE role IS NULL")
